...
// FilmIns.cpp : 응용 프로그램에 대한 클래스 동작을 정의합니다.
//
...
BOOL CFilmInsApp::InitInstance()
{
	...
	if(1 == cmdLine)
		SetPriorityClass(GetCurrentProcess(), HIGH_PRIORITY_CLASS);	//20191120 by 프로세서 우선순위를 실시간으로 변경. (T/T 이점을 얻기 위함.. CPU 자원을 최대한 끌어다 쓰기)
/*
	//프로그램 중복 실행 방지
...
#ifdef _DEBUG
	//_CrtSetBreakAlloc();	//20190828 by 메모리릭 추적 디버깅
#endif
