...
BOOL CSetUpFilmDlg::InspectFilmPF()
{
	...
	for(int i = 0; (i < pdlg->m_dlgSetup->m_InsPara.m_iParaBase[0]) && (i < SUB_RGN_MAX_NUM); i++)
	{
		...
		rslt = pdlg->m_VisMod->FilmPF(pdlg->m_dlgImgRect[m_iRectIdx]->GetView(),			// 결과를 표시할 디스플레이
			pdlg->m_VisMod->GetHalImage(0, pdlg->m_iBufIdx[0], m_map4),						// 이미지 1
			pdlg->m_VisMod->GetHalImage(0, pdlg->m_iBufIdx[0], m_map5),						// 이미지 2
