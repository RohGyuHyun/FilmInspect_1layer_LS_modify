...
void CSetUpOCVDlg::ChageStateInspParamInputBox()
{
	...
	GetDlgItem(IDC_EDIT_TRSH_OCV)->EnableWindow(state);
...
BOOL CSetUpOCVDlg::RegionSet(int map_idx, BOOL auto_mode)
{
	...
	HTuple Row0, Column0, Row1, Column1, Num, Max;
	Hlong wd, ht;
	CRect rect, drect;
	CPoint point;
	HTuple Pointer;
	...
	gen_rectangle1(&Region, pdlg->m_dlgSetup->m_InsPara.m_InsRegionOCV[0][1].top, 
							pdlg->m_dlgSetup->m_InsPara.m_InsRegionOCV[0][1].left, 
							pdlg->m_dlgSetup->m_InsPara.m_InsRegionOCV[0][1].bottom, 
							pdlg->m_dlgSetup->m_InsPara.m_InsRegionOCV[0][1].right);

	reduce_domain(*pdlg->m_VisMod->GetHalImage(0, pdlg->m_iBufIdx[0], map_idx), Region, &ReducedImage); //20110516 lee

	//Find suitable threshold
	min_max_gray(Region, *pdlg->m_VisMod->GetHalImage(0, pdlg->m_iBufIdx[0], map_idx), HTuple(0), _, &Max, _);//20110516 lee
	max = (Hlong)Max[0];

	low_thresh = max * 0.7;//0.5
	high_thresh = max * 0.9;//0.6
	...
	set_check("~give_error");
	herr = hysteresis_threshold(ReducedImage, &RegionHysteresis,HTuple(low_thresh),HTuple(high_thresh),HTuple(20));
	set_check("give_error");
	if(H_MSG_TRUE == herr)
	{
	}
	...
	dilation_rectangle1(RegionHysteresis, &RegionHysteresis,10,15);//4,4);	//20150527 by
	closing_circle(RegionHysteresis,&RegionClosing,HTuple(4.0));//2.0));20141216 lss 
	connection(RegionClosing,&ConnectedRegions);
	sort_region(ConnectedRegions, &ConnectedRegions, "character", "true", "col");
	fill_up(ConnectedRegions,&RegionFillUp);
	closing_rectangle1(RegionFillUp,&RegionClosingRect,HTuple(1),HTuple(50));
...
/*
void CSetUpOCVDlg::ShowSetupPara(int CamIdx)
{
	CFilmInsDlg *pdlg = (CFilmInsDlg *)AfxGetApp()->m_pMainWnd;
	switch(CamIdx)
	{
		case 0:
			if(pdlg->m_dlgSetup->m_SetUpSysDlg.m_25)
			{
				GetDlgItem(IDC_STATIC_GROUP_MK)->ShowWindow(TRUE);

				GetDlgItem(IDC_STATIC_MK1)->ShowWindow(TRUE);
				GetDlgItem(IDC_STATIC_MK2)->ShowWindow(TRUE);
				GetDlgItem(IDC_EDIT_TRSH_OCV)->ShowWindow(TRUE);
				GetDlgItem(IDC_EDIT_SCORE_OCV)->ShowWindow(TRUE);

				GetDlgItem(IDC_BTNENHCTRL_REGION_SET)->ShowWindow(TRUE);
				GetDlgItem(IDC_BTNENHCTRL_OCV_TEACH)->ShowWindow(TRUE);
				GetDlgItem(IDC_BTNENHCTRL_OCV_TEST1)->ShowWindow(TRUE);
			}
			else
			{
				GetDlgItem(IDC_STATIC_GROUP_MK)->ShowWindow(FALSE);

				GetDlgItem(IDC_STATIC_MK1)->ShowWindow(FALSE);
				GetDlgItem(IDC_STATIC_MK2)->ShowWindow(FALSE);
				GetDlgItem(IDC_EDIT_TRSH_OCV)->ShowWindow(FALSE);
				GetDlgItem(IDC_EDIT_SCORE_OCV)->ShowWindow(FALSE);

				GetDlgItem(IDC_BTNENHCTRL_REGION_SET)->ShowWindow(FALSE);
				GetDlgItem(IDC_BTNENHCTRL_OCV_TEACH)->ShowWindow(FALSE);
				GetDlgItem(IDC_BTNENHCTRL_OCV_TEST1)->ShowWindow(FALSE);
			}
			break;
		case 1:
			GetDlgItem(IDC_STATIC_GROUP_MK)->ShowWindow(FALSE);
