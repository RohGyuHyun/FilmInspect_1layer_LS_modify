	...
	while(TRUE)
	{
		if(pdlg->m_bTriggerThread)
		{
			...
			case TRIGGER_THREAD_CONNECTION_STATUS_FUNC:
				pdlg->m_nTriggerStatusCnt = 0;
				while(TRUE)
				{
					pdlg->m_nTriggerStatusCnt++;
					...
					if(pdlg->m_nTriggerStatusCnt == 100)
					{
			...
			case TRIGGER_THREAD_POS_CNT_RESET_FUNC:
				...
				while(TRUE)
				{
					...
					Sleep(10);
					if((pdlg->pUCP->m_VisionTrigger.TrigStatus.CurPosition == 0) && (pdlg->pUCP->m_VisionTrigger.TrigStatus.LScanTrigOut_Counter == 0))
					{
