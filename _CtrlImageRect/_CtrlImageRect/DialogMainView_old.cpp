...
void CDialogMainView::SetZoom(long zoom, long image_width, long image_height, double ExWndZoomRate)
{
	...
	if (ExWndZoomRate != 0.0)
	{
		m_dblWndImgRate = ExWndZoomRate;
	}
	...
	if(1 > zoom)
	{
		...
		m_lZoomStartX = 0;
		m_lZoomStartY = 0;
		m_lZoomEndX = m_WinRect.Width() / m_dblWndImgRate;
		m_lZoomEndY = m_WinRect.Height() / m_dblWndImgRate;
	...
	else if(1 == zoom)
	{
		if(m_bZoom1)
		{
			...
			m_lZoomStartX = 0;
			m_lZoomStartY = 0;
			m_lZoomEndX = m_WinRect.Width() - 1;
			m_lZoomEndY = m_WinRect.Height() - 1;
	...
	else
	{
		...
		zoom_diff = zoom - m_lZoomFact;
		zoom_rat = zoom_diff * 0.001;

//		zoom_part_x = m_lImageWidth * zoom_rat;
//		zoom_part_y = m_lImageHeight * zoom_rat;

		zoom_part_x = (m_WinRect.Width() / m_dblWndImgRate) * zoom_rat;
		zoom_part_y = (m_WinRect.Height() / m_dblWndImgRate) * zoom_rat;

		m_lZoomStartX = m_lZoomStartX + zoom_part_x;
		m_lZoomStartY = m_lZoomStartY + zoom_part_y;

		m_lZoomEndX = m_lZoomEndX - zoom_part_x;
		m_lZoomEndY = m_lZoomEndY - zoom_part_y;
...
void CDialogMainView::GetZoom(long *zoom, long *image_width, long *image_height, long *start_x, long *start_y, long *end_x, long *end_y)  
{
	// TODO: Add your dispatch handler code here
	*zoom = m_lZoomFact;
...
void CDialogMainView::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	...
	for(i = 0; (i < tracker_num) && m_bActive; i++)
	{
		...
		if(NULL != pos)
		{
			...
			switch(tracker->GetType())
			{
				case 0:
				case 1:
					if((m_bCtrlClick && (!check) && tracker->OnLButtonDblClk(nFlags, point)) || (tracker->GetGroup()) )
					{
						check = TRUE;
						m_iActiveIdx = i;
						if(tracker->GetGroupIdx() > -1)
						{
						...
						else
						{
							...
							tracker->SetGroupIdx(i);
						}
					}
					else if(((!check) && tracker->OnLButtonDblClk(nFlags, point)) || (tracker->GetGroup()))
					{
						check = TRUE;
						m_iActiveIdx = i;
						tracker->SetEnable(TRUE);
					...
					{
						if((m_bCtrlClick) && (tracker->GetGroupIdx() > -1))// && tracker->OnLButtonDblClk(nFlags, point)) || (tracker->GetGroup()) )
						{
							tracker->SetEnable(TRUE);
						}
						else if(!tracker->OnLButtonDblClk(nFlags, point))
						{
							tracker->SetEnable(FALSE);
							tracker->SetGroupIdx(-1);
						}
	...
	for(i = 0; (i < tracker_num) && m_bActive; i++)
	{
		...
		{
			tracker = &m_Tracker.GetAt(pos);
			tracker->SetSelectGroupCnt(nSeleCnt);
			m_Tracker.SetAt(pos, *tracker);
		}
	}
...
void CDialogMainView::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	m_bZoom1 = FALSE;
	SetZoom(m_lZoomFact, m_lImageWidth, m_lImageHeight);
	m_bZoom1 = TRUE;
...
void CDialogMainView::ImageDrawLine()
{
	...
	long sx,sy,ex,ey,zoom,width,height;
