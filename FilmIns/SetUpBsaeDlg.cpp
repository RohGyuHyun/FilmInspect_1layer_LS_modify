...
BOOL CSetUpBsaeDlg::OnInitDialog()
{
	...
	case VISION_CAM_TOP1:
		...
		//20150224 ngh
		/*m_SetUpChipDlg.ShowSetupPara(VISION_CAM_TOP1);
		m_SetUpFilmDlg.ShowSetupPara(VISION_CAM_TOP1);
		m_SetUpLeadDlg.ShowSetupPara(VISION_CAM_TOP1);
		m_SetUpSRDlg.ShowSetupPara(VISION_CAM_TOP1);
		m_SetUpOCVDlg.ShowSetupPara(VISION_CAM_TOP1);*/
		break;
	case VISION_CAM_TOP2:
		...
		hTreeItem1 = m_Tree.InsertItem("(상부)A/M", 0, 1, TVI_ROOT, TVI_LAST);
		m_Tree.SetItemData(hTreeItem1, (DWORD)&m_SetUpLeadDlg);
		...
		//20150224 ngh
		/*m_SetUpChipDlg.ShowSetupPara(VISION_CAM_TOP2);
		m_SetUpFilmDlg.ShowSetupPara(VISION_CAM_TOP2);
		m_SetUpLeadDlg.ShowSetupPara(VISION_CAM_TOP2);
		m_SetUpSRDlg.ShowSetupPara(VISION_CAM_TOP2);
		m_SetUpOCVDlg.ShowSetupPara(VISION_CAM_TOP2);*/
		break;
	case VISION_CAM_BOTTOM:
		...
		//20150224 ngh
		/*m_SetUpChipDlg.ShowSetupPara(VISION_CAM_BOTTOM);
		m_SetUpFilmDlg.ShowSetupPara(VISION_CAM_BOTTOM);
		m_SetUpLeadDlg.ShowSetupPara(VISION_CAM_BOTTOM);
		m_SetUpSRDlg.ShowSetupPara(VISION_CAM_BOTTOM);
		m_SetUpOCVDlg.ShowSetupPara(VISION_CAM_BOTTOM);*/
		break;
	default:
	...
	/*
	//20190918 by Potting 관련 파라미터 숨기기
	if(VISION_CAM_TOP2 == pdlg->m_VisMod->GetVisIndex())
	{
		rcDlgs2.bottom -= 200;
		m_SetUpChipDlg.MoveWindow(rcDlgs2); 
	}
	else
	{
	...
	*/
	...
	m_ctrlPFCnt.InsertString(9, "11 PF");
	m_ctrlPFCnt.InsertString(10, "12 PF");
	m_ctrlPFCnt.InsertString(11, "13 PF");
	m_ctrlPFCnt.InsertString(12, "14 PF");
	m_ctrlPFCnt.InsertString(13, "15 PF");
	m_ctrlPFCnt.InsertString(14, "16 PF");

	m_ctrlCOFWidth.InsertString(0, "35 mm");
	m_ctrlCOFWidth.InsertString(1, "48 mm");
	m_ctrlCOFWidth.InsertString(2, "70 mm");

	//lby 20141022 systempara 데이터를 여기서 얻어야함. 기존에는 없었음.
	m_InsPara = pdlg->m_VisMod->GetInspectPara(0);
	m_SysPara = pdlg->m_VisMod->GetSystemPara(0);

	//20150223 ngh	setup 창 디스플레이 영역 뒤로 숨는것 방지
	GetWindowRect(&m_FixeRectPos);
...
void CSetUpBsaeDlg::ClickBtnenhctrlSetupBaseApplyall()
{
	...
	FILM_LEFT = (WIDTH - (pdlg->m_dlgSetup->m_InsPara.m_dParaBase[2] / pdlg->m_dlgSetup->m_SysPara.m_dPara[0])) * 0.5 + 4;	// 2 pixel margin
	FILM_RIGHT = FILM_LEFT + (pdlg->m_dlgSetup->m_InsPara.m_dParaBase[2] / pdlg->m_dlgSetup->m_SysPara.m_dPara[0]) - 8;
...
void CSetUpBsaeDlg::OnTvnSelchangedTree1(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	CFilmInsDlg *pdlg = (CFilmInsDlg *)AfxGetApp()->m_pMainWnd;
	int cam_idx = pdlg->m_VisMod->GetVisIndex();
...
void CSetUpBsaeDlg::Update(BOOL flag)
{
	...
	else
	{
		...
		{
		case VISION_CAM_TOP1:
			pdlg->m_dlgSetup->m_InsPara.m_iParaBase[1] = USE_MAP_CNT_VIS_LINE1;
			break;
		case VISION_CAM_TOP2:
				pdlg->m_dlgSetup->m_InsPara.m_iParaBase[1] = USE_MAP_CNT_VIS_LINE2;
			break;
		case VISION_CAM_BOTTOM:
				pdlg->m_dlgSetup->m_InsPara.m_iParaBase[1] = USE_MAP_CNT_VIS_LINE3;
			break;
		}
	}
}
...
void CSetUpBsaeDlg::OnCbnSelchangePfCnt()
{
	...
	case 8:
		...
		break;
	case 9:
		pdlg->m_dlgSetup->m_InsPara.m_iParaBase[2] = 11;
		m_cof_num = 1;
		break;
	case 10:
		pdlg->m_dlgSetup->m_InsPara.m_iParaBase[2] = 12;
		m_cof_num = 1;
		break;
	case 11:
		pdlg->m_dlgSetup->m_InsPara.m_iParaBase[2] = 13;
		m_cof_num = 1;
		break;
	case 12:
		pdlg->m_dlgSetup->m_InsPara.m_iParaBase[2] = 14;
		m_cof_num = 1;
		break;
	case 13:
		pdlg->m_dlgSetup->m_InsPara.m_iParaBase[2] = 15;
		m_cof_num = 1;
		break;
	case 14:
		pdlg->m_dlgSetup->m_InsPara.m_iParaBase[2] = 16;
		m_cof_num = 1;
		break;
	}
...
void CSetUpBsaeDlg::UpdateComBox(int COFWidth, int PFCnt)
{
	...
	case 10:
		...
		break;
	case 11:
		m_ctrlPFCnt.SetCurSel(9);
		break;
	case 12:
		m_ctrlPFCnt.SetCurSel(10);
		break;
	case 13:
		m_ctrlPFCnt.SetCurSel(11);
		break;
	case 14:
		m_ctrlPFCnt.SetCurSel(12);
		break;
	case 15:
		m_ctrlPFCnt.SetCurSel(13);
		break;
	case 16:
		m_ctrlPFCnt.SetCurSel(14);
		break;
	}
}
