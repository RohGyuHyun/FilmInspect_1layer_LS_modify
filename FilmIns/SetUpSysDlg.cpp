...
void CSetUpSysDlg::Update(BOOL flag)
{
	...
	if(flag)
	{
		...
		case VISION_CAM_TOP1:	// 상부#1 고정 검사 항목
			...
			m_29 = FALSE;		// "
			break;
		case VISION_CAM_TOP2:	// 상부#2 고정 검사 항목
			m_0 = TRUE;
			m_2 = TRUE;
			...
			m_29 = FALSE;		// "
			break;
		case VISION_CAM_BOTTOM:	// 하부#1 고정 검사 항목
			m_27 = TRUE;		//기포,잉크칩
			m_28 = TRUE;		//구겨짐,찍힘
			m_29 = TRUE;		// "
		...
		//m_2 = pdlg->m_dlgSetup->m_SysPara.m_bPara[2];	//Chip 수지, Broken
	...
	else
	{
		...
		case VISION_CAM_TOP1:	// 상부#1 고정 검사 항목
			...
			pdlg->m_dlgSetup->m_SysPara.m_bPara[2] = FALSE;	//Chip 수지, Broken
		...
		case VISION_CAM_TOP2:	// 상부#2 고정 검사 항목
			...
			pdlg->m_dlgSetup->m_SysPara.m_bPara[2] = TRUE;	//Chip 수지, Broken
		...
		//pdlg->m_dlgSetup->m_SysPara.m_bPara[2] = m_2;	//Chip 수지, Broken
...
BOOL CSetUpSysDlg::OnInitDialog()
{
	...
	switch(pdlg->m_VisMod->GetVisIndex())
	{
		case VISION_CAM_TOP1:
			GetDlgItem(IDC_CHECK_INS_USE0_SYS)->EnableWindow(FALSE);
			GetDlgItem(IDC_CHECK_INS_USE2_SYS)->EnableWindow(FALSE);

			GetDlgItem(IDC_CHECK_INS_USE15_SYS)->EnableWindow(FALSE);
			GetDlgItem(IDC_CHECK_INS_USE16_SYS)->EnableWindow(FALSE);
			break;
		case VISION_CAM_TOP2:
			GetDlgItem(IDC_CHECK_INS_USE0_SYS)->EnableWindow(TRUE);
			GetDlgItem(IDC_CHECK_INS_USE2_SYS)->EnableWindow(TRUE);

			GetDlgItem(IDC_CHECK_INS_USE20_SYS)->EnableWindow(FALSE);
			GetDlgItem(IDC_CHECK_INS_USE21_SYS)->EnableWindow(FALSE);
			GetDlgItem(IDC_CHECK_INS_USE22_SYS)->EnableWindow(FALSE);
			GetDlgItem(IDC_CHECK_INS_USE23_SYS)->EnableWindow(FALSE);
			GetDlgItem(IDC_CHECK_INS_USE25_SYS)->EnableWindow(FALSE);
			break;
	}
