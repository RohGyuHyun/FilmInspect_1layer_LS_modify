...
BOOL CFilmInsDlg::OnInitDialog()
{
	...
	SetLightIndex(m_VisMod->GetVisIndex(), m_bLineCamOn);	 											// 조명 인덱스

	SetStaticMapType(m_VisMod->GetVisIndex(), m_bLineCamOn);										// 검사 항목 인덱스
...
void CFilmInsDlg::InitCamType(int nType)
{
	...
	m_strDBPath.Format("D:\\DATABASE_%d\\MODEL", nType);				// 모델 저장 경로 지정
	m_strRegiPath.Format("SOFTWARE\\FILMINS_%d\\MODEL", nType);		// 모델 레지스트리 경로 지정
	m_strLogPath.Format("E:\\Insp_Result_%d", nType);					//로그, 이미지 저장경로
	...
	case VISION_CAM_TOP1:
		strTemp.Format("::CAM #%d(상부) 1 METAL", m_VisMod->GetVisIndex() + 1);			// 디스플레이 제목표시줄 이름
	...
	case VISION_CAM_TOP2:
		strTemp.Format("::CAM #%d(상부) 1 METAL", m_VisMod->GetVisIndex() + 1);
	...
	case VISION_CAM_BOTTOM:
		strTemp.Format("::CAM #%d(하부) 1 METAL", m_VisMod->GetVisIndex() + 1);
...
//	typeVisionResultData VisRsltData;
				...
				if(Hal_OpenHalImage(m_VisMod->GetHalImage((int)para0, m_iBufIdx[0], m_iMapIdx[0]), "", FALSE))
				{
					...
					*/
					...
					if(m_bIsColor)
					{
						...
						//20190909 by 최초 영상 취득 시 회전 시킨 상태이므로 사용하지 않음. 주석처리함.
						//if(m_bLineCamOn)
	...
	case 4:
		...
		{
		case 0:
			//20190909 by 최초 저장 시 라인스캔 컬러 이미지 용량 문제로 jpeg으로 저장함.
			if(m_bLineCamOn)
...
void CFilmInsDlg::InspectProcess(int cam_idx, int prc_idx, int map_Idx)
{
	...
	case VISION_CAM_TOP1:
		...
		disp_obj(*m_VisMod->GetHalImage(0, prc_idx, map_Idx), m_dlgImgRect[0]->GetView());			//20191204 by 카메라 별 이미지 출력 구분 추가
		break;
	case VISION_CAM_TOP2:
		...
		if(1 != m_iRslt[0])
		{
			//NG 검사 결과를 리스트에 담는다			//20191101 by
			m_VisMod->PushRsltData(0);
		}
...
//	shift_rslt = m_VisMod->GetShiftRslt(0, buf_idx);
	...
	case 1:
		...
		m_VisMod->SetInspectNGRsltRgn(0, m_iNGViewIdx[0]);
		//Set result data
		m_VisMod->SetInspectNGRslt(0, m_iNGViewIdx[0], /*shift_rslt,*/ ins_rslt); // 디버그 필요 (vision module 에서 shift_rslt 제외 할것)
...
void CFilmInsDlg::OnOffLight(BOOL LightOn, BOOL bUseLine)
{
	if(bUseLine)
	{
		if(m_cSerial.IsOpen())
		{
			...
			//20190910 by 스테코에서 개조 진행했던 조명 프로토콜
			/*
			if(LightOn)
...
void CFilmInsDlg::SetLineLight(int ch, int var)
{
	if(m_cSerial.IsOpen())
	{
		...
		//sprintf(data, "setbrightness %d %d%c", ch, var, 0x0D);	//20190910 by 스테코에서 개조 진행했던 조명 프로토콜
...
void CFilmInsDlg::ResultCropImg(Hobject *Img, int cof_idx, int nIdx, int ngtype)
{
	...
	if(sys_para.m_bPara[31])
	{
		...
		//20200802 by 부분 NG 이미지 저장 품질 상향
...
void CFilmInsDlg::ResultCheck_Display(int cam_idx, int buf_idx, int ng_view_idx, BOOL auto_mode, BOOL main_view, BOOL chk_ng_type, BOOL chk_ng_Disp, BOOL ng_view_data_save) //20220509 lee
{
	...
	{	
		...
		case VISION_CAM_TOP2:
			...
			nStep = 350;
			break;
			//nCol = nCol + nStep;  이 녀석을 따라가면서 수정하기.	20190930 by IMSI
		}
			...
			else
			{
				...
				else
				{
					...
					if(ScrollCheck(cur_scroll_idx))
					{
						...
						strCaption.Format("CHIP #%d NG : ALIGN 인식실패", i + 1);
			...
			else
			{
				...
				else
				{
					...
					if(ScrollCheck(cur_scroll_idx))
					{
						...
						strCaption.Format("CHIP #%d NG : ALIGN 인식실패-NO DATA", i + 1);
						write_string(m_lNGWindowID[m_iNGSubViewIdx[0]], HTuple(strCaption));
			...
			else
			{
				...
				else
				{
					...
					if(ScrollCheck(cur_scroll_idx))
					{
						...
						strCaption.Format("CHIP #%d NG : ALIGN 인식실패-POS.LMT.", i + 1);
						write_string(m_lNGWindowID[m_iNGSubViewIdx[0]], HTuple(strCaption));
			...
			else
			{
				...
				else
				{
					...
					if(ScrollCheck(cur_scroll_idx))
					{
						...
						strCaption.Format("CHIP #%d NG : PF막힘/PUNCH BURR(편집불량)", i + 1);
						write_string(m_lNGWindowID[m_iNGSubViewIdx[0]], HTuple(strCaption));
...
*/
				...
				else
				{
					...
					for(j = 0; (j < ins_ng_rslt.m_InsRslt.m_iChipMoldNum[i]) && (j < NG_MAX_NUM) && (j < rslt_num) ; j++)
					{
						...
						if(ScrollCheck(cur_scroll_idx))
						{
							...
							strCaption.Format("CHIP #%d NG : Chip 이면수지묻음", i + 1);
							write_string(m_lNGWindowID[m_iNGSubViewIdx[0]], HTuple(strCaption));
		...
		case -26:	//포팅 길이 불량 추가20150818 by 
				...
				else
				{
					...
					for(j = 0; (j < ins_ng_rslt.m_InsRslt.m_iChipPotRNum[i]) && (j < NG_MAX_NUM) && (j < rslt_num) ; j++)
					{
						...
						if(ScrollCheck(cur_scroll_idx))
						{
							...
							strCaption.Format("CHIP #%d NG : 도포범위불량", i + 1);
							write_string(m_lNGWindowID[m_iNGSubViewIdx[0]], HTuple(strCaption));
				...
				else
				{
					...
					for(j = 0; (j < ins_ng_rslt.m_InsRslt.m_iFilmMoldNum[i]) && (j < NG_MAX_NUM) && (j < rslt_num) ; j++)
					{
						...
						if(ScrollCheck(cur_scroll_idx))
						{
							...
							strCaption.Format("CHIP #%d NG : FILM 수지/잉크 묻음/긁힘", i + 1);
							write_string(m_lNGWindowID[m_iNGSubViewIdx[0]], HTuple(strCaption));
				...
				else
				{
					...
					for(j = 0; (j < ins_ng_rslt.m_InsRslt.m_iFilmPFNum[i]) && (j < NG_MAX_NUM) && (j < rslt_num) ; j++)
					{
						...
						if(ScrollCheck(cur_scroll_idx))
						{
							...
							strCaption.Format("CHIP #%d NG : FILM PF 찢김/막힘", i + 1);
							write_string(m_lNGWindowID[m_iNGSubViewIdx[0]], HTuple(strCaption));
				...
				else
				{
					...
					for(j = 0; (j < ins_ng_rslt.m_InsRslt.m_iFilmMetalNum[i]) && (j < NG_MAX_NUM) && (j < rslt_num) ; j++)
					{
						...
						if(ScrollCheck(cur_scroll_idx))
						{
							...
							strCaption.Format("CHIP #%d NG : SR면 METAL PART.", i + 1);
							write_string(m_lNGWindowID[m_iNGSubViewIdx[0]], HTuple(strCaption));
				...
				else
				{
					...
					for(j = 0; (j < ins_ng_rslt.m_InsRslt.m_iLeadTopNum[i]) && (j < NG_MAX_NUM) && (j < rslt_num) ; j++)
					{
						...
						if(ScrollCheck(cur_scroll_idx))
						{
							...
							set_color(m_lNGWindowID[m_iNGSubViewIdx[0]], HTuple("red"));
							set_tposition(m_lNGWindowID[m_iNGSubViewIdx[0]], sy + 10, sx + 10);
							strCaption.Format("CHIP #%d NG : LEAD 변색/결함(OUT)", i + 1);
							write_string(m_lNGWindowID[m_iNGSubViewIdx[0]], HTuple(strCaption));
				...
				else
				{
					...
					for(j = 0; (j < ins_ng_rslt.m_InsRslt.m_iLeadBotNum[i]) && (j < NG_MAX_NUM) && (j < rslt_num) ; j++)
					{
						...
						if(ScrollCheck(cur_scroll_idx))
						{
							...
							strCaption.Format("CHIP #%d NG : LEAD 변색/결함(IN)", i + 1);
							write_string(m_lNGWindowID[m_iNGSubViewIdx[0]], HTuple(strCaption));
		...
		case 1:
			...
			{
				//검사영역을 NG view 로 표시한다. 20150807 by
				...
				//양품 로그
				//ResultAMLog(cam_idx, buf_idx, ng_view_idx, i, j, (Hlong)Mean[0]);// 20160303 by 보류
							/*
							//티칭 값
				...
				else	//NG View
				{
					...
					for(j = 0; (j < ins_ng_rslt.m_InsRslt.m_iLeadAMNum[i]) && (j < NG_MAX_NUM) && (j < rslt_num) ; j++)
					{
						...
						if(ScrollCheck(cur_scroll_idx))
						{
							...
							///AM 불량 발생시 Area 사이즈 출력
							set_tposition(m_lNGWindowID[m_iNGSubViewIdx[0]], (Hlong)(sy + (20*interval)), sx);
							area_center(SelectRgn, &Area, _, _);
							write_string(m_lNGWindowID[m_iNGSubViewIdx[0]], HTuple( HTuple("영역크기 : ") + HTuple((Hlong)Area[0])) );
							...
							write_string(m_lNGWindowID[m_iNGSubViewIdx[0]], HTuple( HTuple("평균밝기 : ") + HTuple((Hlong)Mean[0])) );
...
//						for(j = 0; (j < ins_ng_rslt.m_InsRslt.m_iOCVPatNum[i]) && (j < NG_MAX_NUM) && (j < rslt_num) ; j++)
				...
				else
				{
					...
					if(ScrollCheck(cur_scroll_idx))
					{
						...
						strCaption.Format("CHIP #%d NG : 인쇄품질-NO CHARS.", i + 1);
						write_string(m_lNGWindowID[m_iNGSubViewIdx[0]], HTuple(strCaption));
			...
			else
			{
				...
				else
				{
					...
					if(ScrollCheck(cur_scroll_idx))
					{
						...
						strCaption.Format("CHIP #%d NG : 인쇄품질-NO DATA", i + 1);
						write_string(m_lNGWindowID[m_iNGSubViewIdx[0]], HTuple(strCaption));
				...
				else
				{
					...
					for(j = 0; (j < ins_ng_rslt.m_InsRslt.m_iOCVPatNum[i]) && (j < NG_MAX_NUM) && (j < rslt_num) ; j++)
					{
						...
						if(ScrollCheck(cur_scroll_idx))
						{
							...
							strCaption.Format("CHIP #%d NG : 인쇄품질-NO CHAR.", i + 1);
							write_string(m_lNGWindowID[m_iNGSubViewIdx[0]], HTuple(strCaption));
			...
			else
			{
				...
				if(main_view)
				{
					...
					*/
					...
					write_string(m_dlgImgRect[cam_idx]->GetView(), "NG : PI 기포/잉크칩");
				...
				else
				{
					...
					for(j = 0; (j < ins_ng_rslt.m_InsRslt.m_iSRBubNum[i]) && (j < NG_MAX_NUM) && (j < rslt_num) ; j++)
					{
						...
						if(ScrollCheck(cur_scroll_idx))
						{
							...
							//20150715 by
							//잉크칩 검사는 Green  이미지로 하기 때문에 Green 으로 출력
							//잉크칩 검출 사이즈 표시
							...
							set_color(m_lNGWindowID[m_iNGSubViewIdx[0]], HTuple("red"));
							set_tposition(m_lNGWindowID[m_iNGSubViewIdx[0]], sy + 10, sx + 10);
							//write_string(m_lNGWindowID[m_iNGSubViewIdx[0]], "NG : PI 기포/잉크칩");
							strCaption.Format("CHIP #%d NG : PI 기포/잉크칩", i + 1);
							write_string(m_lNGWindowID[m_iNGSubViewIdx[0]], HTuple(strCaption));
				...
				else
				{
					...
					for(j = 0; (j < ins_ng_rslt.m_InsRslt.m_iPIHallNum[i]) && (j < NG_MAX_NUM) && (j < rslt_num) ; j++)
					{
						...
						if(ScrollCheck(cur_scroll_idx))
						{
							...
							strCaption.Format("CHIP #%d NG : PI 찍힘/구겨짐", i + 1);
							write_string(m_lNGWindowID[m_iNGSubViewIdx[0]], HTuple(strCaption));
	...
	for(int i = 0; i < nCount; i++)
	{
		...
		else if(!strCmd.Compare("0017"))				//Lot Start
		{
			...
			if(m_bLineCamOn)
			{
				...
				//20191101 by NG 정보 리스트 관리 변수 초기화
				m_nNGIndex = 0;
				m_bNGBegin = TRUE;
		...
		else if(!strCmd.Compare("0019"))				//Lot End
		{
			...
			if(m_VisMod->GetVisIndex() == VISION_CAM_TOP2)
			{
				if(m_bLineCamOn)
				{
					...
					//20191230 by NG 이미지 폴더를 복사

					CString fromPath;
					CString toPath;
					
					fromPath.Format("%s\\%s\\%s\\IMAGE\\NG", m_strLogPath, m_strStartTime, m_strLotNo);
					toPath.Format("z:\\IMAGE\\NG");	// ==> 온라인베리파이측 경로 확인하여 입력
		...
		else if(!strCmd.Compare("0027"))		//트리거 위치 초기화
		{
			m_cTriggerBoard->SetTrigger_Trig_Pos_Reset();
...
void CFilmInsDlg::OnRectImgMap0()
{
	...
	{
		//함수인자(int cam_idx, int buf_idx, int ng_view_idx, BOOL auto_mode, BOOL main_view, BOOL chk_ng_type, BOOL chk_ng_Disp, BOOL ng_view_data_save)
...
void CFilmInsDlg::ClickBtnenhctrlSetup()
{
	...
	if(dlg.DoModal() == IDOK)
	{
		...
		if(0 == dlg.m_ctrlPassword.Compare(sys_para.m_strPara))
		{
			...
			//20150223 ngh	setup 창 디스플레이 영역 뒤로 숨는것 방지
			if(m_dlgSetup->IsWindowVisible())
			{
...
void CFilmInsDlg::SetStaticMapType(int vis_idx, BOOL isLineScan)
{
	...
	case VISION_CAM_TOP1:
		...
		GetDlgItem(IDC_STATIC_MAP0_TYPE1)->SetWindowText("Punch불량");
		GetDlgItem(IDC_STATIC_MAP0_TYPE2)->SetWindowText("PF불량");
		GetDlgItem(IDC_STATIC_MAP0_TYPE3)->SetWindowText("Film찢김");

		GetDlgItem(IDC_STATIC_MAP1_TYPE0)->SetWindowText("Potting");
		GetDlgItem(IDC_STATIC_MAP1_TYPE1)->SetWindowText("No Bond");
		//GetDlgItem(IDC_STATIC_MAP1_TYPE2)->SetWindowText("Chip수지묻음/Broken");		
	...
	case VISION_CAM_TOP2:
		...
		GetDlgItem(IDC_STATIC_MAP0_TYPE3)->SetWindowText("Chip 이면수지묻음");

		//GetDlgItem(IDC_STATIC_MAP1_TYPE0)->SetWindowText("Chip Out/Broken");					//IMSI by 촬상 수 조정
		//GetDlgItem(IDC_STATIC_MAP2_TYPE1)->SetWindowText("Chip 이면수지묻음");
		break;
	case VISION_CAM_BOTTOM:
		GetDlgItem(IDC_STATIC_MAP0_TYPE0)->SetWindowText("Align");
		...
		//GetDlgItem(IDC_STATIC_MAP3_TYPE0)->SetWindowText("편집불량");
...
{
	//삭제 기일이 0 일 경우 아무것도 하지 않는다. 20150821 by
...
{
	// TODO: 여기에 메시지 처리기 코드를 추가합니다.
	if(IDYES == AfxMessageBox("자동 활자 티칭을 진행할까요?", MB_YESNO))
	{
...
void CFilmInsDlg::DisplayDeviceGood(Hobject *RsltRegion, int cur_scroll_idx, int ng_view_idx, int disp_map_num)
{
	...
	if(ScrollCheck(cur_scroll_idx))
	{
		...
		decompose3(*m_VisMod->GetHalNGImage(0, ng_view_idx, disp_map_num), &obj_red, &obj_green, &obj_blue);		//20191203_2 by 테스트 중
...
void CFilmInsDlg::CopyDirectory(CString defPath, CString copyTo)
{
	...
	while (bRlt) {
		...
		if (cFileFinder.IsDirectory()) {
			...
			CopyDirectory(defPath + "\\" + fName, copyTo + "\\" + fName); // 하위 폴더 확인
		}
