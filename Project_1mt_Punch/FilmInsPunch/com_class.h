	...
	DWORD Read(LPVOID Buffer, DWORD dwBufferLength, DWORD dwWaitTime = 20)
	{
		...
		if(!::ReadFile(_hCommHandle, Buffer, dwBufferLength, &uReadLength, &_ReadOverlapped)) 
		{
			if(::GetLastError() == ERROR_IO_PENDING) 
			{
				...
				if(!::GetOverlappedResult(_hCommHandle, &_ReadOverlapped, &uReadLength, true))
				{
...
protected:
	...
	void Init() 
	{
		...
		_ReadOverlapped.hEvent = CreateEventA(NULL, true, false, NULL);
		assert(_ReadOverlapped.hEvent != INVALID_HANDLE_VALUE); 
		
		_WriteOverlapped.hEvent = CreateEventA(NULL, true, false, NULL);
		...
		_WaitOverlapped.hEvent = CreateEventA(NULL, true, false, NULL);
